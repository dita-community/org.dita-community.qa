...
<xsl:for-each select="/topic/body/section[contains(@outputclass, 'topicReport')]"><xsl:for-each select="data/descendant-or-self::*[contains(@type, 'msg')][not(contains(@outputclass, 'step'))][not(contains(@outputclass, 'xrefs'))]">
<xsl:value-of select="concat('=HYPERLINK(&quot;', preceding::data[contains(@name, 'filePath')][1], '&quot;,&quot;', preceding::data[contains(@name, 'topicTitle')][1]/normalize-space(replace(.,',',';')), ' [click to open]&quot;)')"/><xsl:value-of select="$delim"/><xsl:value-of select="preceding::data[contains(@name, 'filePath')][1]"/><xsl:value-of select="$delim"/><xsl:value-of select="replace(normalize-space(.), ',',';')"/><xsl:value-of select="$delim"/><xsl:value-of select="replace(@outputclass,',',';')"/><xsl:value-of select="$delim"/><xsl:value-of select="@importance"/><xsl:text>
