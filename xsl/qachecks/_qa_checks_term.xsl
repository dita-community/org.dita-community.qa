...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
   ...
   <xsl:template name="term">
      <xsl:if test="b">
         <data type="msg" class="" importance="warning">Do not use typographic markup b</data>
      </xsl:if>
      <xsl:if test="p[(not(text()) and not(node()))]">
         <data type="msg" class="" importance="error">P element is empty</data>
      </xsl:if>
      <xsl:if test="descendant::*[not(codeblock or draft-comment or filepath or shortdesc or uicontrol or varname)]/text()[matches(.,'data center', 'i')]">
         <data type="msg" class="" importance="warning">data center - Use "datacenter"</data>
      </xsl:if>
      <xsl:if test="descendant::*[not(codeblock or draft-comment or filepath or shortdesc or uicontrol or varname)]/text()[matches(.,'via', 'i')]">
         <data type="msg" class="" importance="error">Avoid "via." Use "across," "along," "by," "from," "on,"
